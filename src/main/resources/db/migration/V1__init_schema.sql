...
(
    ...
    BASE_FARE_AMT DECIMAL(15,2) NOT NULL COMMENT '기본_운임_금액 — 세금·할증 완전 제외 순수 운임. 고객 총 결제액 = 이 값 + TBL_FARE_TAX 합계',
...
CREATE INDEX IDX_FARE_LOOKUP ON TBL_FARE (ARLN_CD, DPTRE_ARPT_CD, ARVL_ARPT_CD, VLD_BGNG_DT, VLD_END_DT);


-- ==========================================================================================
-- 세금/할증 테이블 (TBL_FARE_TAX)
...
(
    ...
    TXBS_CD       VARCHAR(20)   DEFAULT 'BASE_FARE' NOT NULL COMMENT '과세_기준_코드 — PCT 유형의 곱셈 기준값 결정. BASE_FARE=기본운임만, TOTAL=직전 단계까지의 누적 합계. TBL_FARE_TAX_MAPNG.APLCN_ORD 순서와 반드시 함께 해석해야 함',
    ...
    VSRN_ID       VARCHAR(36)   NOT NULL COMMENT '버전_식별자 — UUID. TBL_FARE 와 동일한 재현성 추적 방식',
    REG_DT        DATETIME      DEFAULT CURRENT_TIMESTAMP NOT NULL COMMENT '등록_일시',
    RGTR_ID       VARCHAR(30)   NOT NULL COMMENT '등록자_아이디',
...
CREATE INDEX IDX_FARE_TAX_CD ON TBL_FARE_TAX (TAX_CD, VLD_BGNG_DT, VLD_END_DT);


-- ==========================================================================================
-- 운임-세금 매핑 테이블 (TBL_FARE_TAX_MAPNG)
-- 운임:세금 N:M 연결 + APLCN_ORD 로 계산 순서 제어
-- APLCN_ORD 가 낮은 번호부터 계산. PCT+TXBS_CD='TOTAL' 은 앞 순서 세금까지 누적한 값에 곱함
-- ==========================================================================================

CREATE TABLE TBL_FARE_TAX_MAPNG
(
    FARE_TAX_MAPNG_ID VARCHAR(30) NOT NULL COMMENT '운임_세금_매핑_식별자'                                                                             PRIMARY KEY,
    FARE_ID           VARCHAR(30) NOT NULL COMMENT '운임_식별자 — TBL_FARE.FARE_ID 참조. 해당 운임에 이 세금을 적용',
    FARE_TAX_ID       VARCHAR(30) NOT NULL COMMENT '세금_할증_식별자 — TBL_FARE_TAX.FARE_TAX_ID 참조',
...
ALTER TABLE TBL_FARE_TAX_MAPNG
    ADD CONSTRAINT FK_FARE_TAX_MAPNG_FARE
        FOREIGN KEY (FARE_ID) REFERENCES TBL_FARE (FARE_ID);

ALTER TABLE TBL_FARE_TAX_MAPNG
    ADD CONSTRAINT FK_FARE_TAX_MAPNG_TAX
        FOREIGN KEY (FARE_TAX_ID) REFERENCES TBL_FARE_TAX (FARE_TAX_ID);

-- 동일 운임에 동일 세금 중복 방지
CREATE UNIQUE INDEX UK_FARE_TAX_MAPNG ON TBL_FARE_TAX_MAPNG (FARE_ID, FARE_TAX_ID);
